...
from django.db import transaction # Атомарный запрос
...
def film_list_create_api_view(request):
    if request.method == 'GET':
        ...
        # step 2: Create film 
        # (Film.objects.create(t=t,.. ) и film.save()-в конце )- работают одинаково
        with transaction.atomic(): # Атомарный запрос
            ...
            film.save()
            # Film.objects.bulk_create([film, film, film]) / Это если нужно вывесть список фильмов (если много)
        
        # step 3: Return response(вернуть ответ)
        return Response(
            status=status.HTTP_201_CREATED,
            data=FilmDetailSerializer(film).data
        )